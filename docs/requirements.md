...
**Correction to the paragraph above:** it says `scripts/schema.sql` adds `city`/`ward` via `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`. That turned out to be wrong in a way that mattered — `IF NOT EXISTS` on `ALTER TABLE ADD COLUMN`/`ADD INDEX` is a MariaDB-only extension; real MySQL 8.0 rejects it as a syntax error (see §9 below). It was only ever tested against MariaDB, which happened to accept it, masking the incompatibility. Superseded by `ensureCityWardColumns()` in `scripts/migrate.ts`, which checks `information_schema` first.

## 9. Round-2/round-3 QC passes, MySQL-8-compatibility fix, and the search bar moved to the top

Three more rounds of work landed on top of §8, all against the same MySQL-backed `db.ts`:

**Round 2 (independent adversarial QA pass):** found and fixed one CRITICAL and three MEDIUM issues, all specific to the MySQL join code and to routes that hadn't been checked for this exact vulnerability class before. CRITICAL: `listRooms()`'s join query (`SELECT r.*, p.*`) collided on column names the two tables share (`id`, `images`, `is_active`, `created_at`, `updated_at`) — `mysql2` silently keeps only the last occurrence, so every room returned actually carried the *property's* images/active-flag/timestamps, meaning every room card on the public site showed the property's cover photo instead of the room's own. This predates this cycle's work — it was already present in the original MySQL migration — but is fixed now: the query uses `{ sql, nestTables: true }` so each table's columns land in their own namespace regardless of overlap. The three MEDIUM findings were all the same vulnerability class in different places: `GET /api/properties`, `GET /api/properties/[id]`, `GET /api/rooms/[id]`, and the public `/rooms/[id]` page all let a deactivated (soft-deleted) property or room stay fully retrievable by anyone who had or guessed its id/URL, bypassing the "unlisted from the public site" intent of deactivation entirely. All four now return 404 to non-admin callers, while an admin session still sees the record (needed to review/reactivate it).

**Origin reconciliation + a genuine MySQL-8 portability fix:** before starting round 3, a routine `git fetch` turned up a new commit on `origin/main` from a different session/tool. Diffing it (not trusting the commit message) showed it was byte-for-byte identical to this cycle's round-1+round-2 work, plus exactly one additional, verified improvement: real MySQL 8.0 rejects `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` / `ADD INDEX IF NOT EXISTS` with a syntax error — that syntax is a MariaDB-only extension, and this cycle's earlier testing only ever ran against MariaDB (which accepts it), so the incompatibility went unnoticed. Fixed by moving the "if not exists" check into `ensureCityWardColumns()` in `scripts/migrate.ts`: it reads `information_schema.COLUMNS`/`STATISTICS` first and only runs a plain `ALTER TABLE` when the column/index is actually missing — portable across both MySQL and MariaDB. **This specific claim (that MySQL 8.0 truly rejects the old syntax) rides on the other commit's testing, not this cycle's** — there was no real MySQL 8.0 instance available in this sandbox to reproduce the exact syntax error directly, only MariaDB. Worth an explicit double-check against whatever MySQL/MariaDB version actually runs on the production VPS before treating it as fully confirmed, though the rewritten code is safe either way regardless of which server it hits.

**UI: search bar moved to the top of the page.** The owner found it inconvenient that the primary filters (free-text address search, Tỉnh/Thành phố dropdown, Phường/Xã dropdown) were buried in the left sidebar below the secondary status/price filters. `src/components/MainSearchBar.tsx` is a new component holding just those three controls, rendered full-width above the listing grid; `FilterBar.tsx` now holds only the secondary refinements (status pills, price-bucket pills) and stays in a narrower sidebar. Screenshot-verified with a headless browser at both desktop and phone widths — the phone layout needed one follow-up fix (the two dropdowns were squeezed into a row and their labels got truncated; they now stack vertically below the `sm` breakpoint).

**Round 3 (independent adversarial QA pass, again a fresh reviewer uninvolved in writing the reviewed code):** targeted the three commits above plus a fresh look at the money-handling code. Two MEDIUM findings, both fixed:
1. `validatePropertyMoneyFields()` checked `landlordSharePercent` and `saleSharePercent` (the cancellation-settlement split) were each independently within 0–100%, but never that they summed to 100. `calculateCancellationSettlement()` only ever reads `saleSharePercent` and derives the landlord's share as the complement (for correct rounding) — it never reads `landlordSharePercent` at all. So an admin could save inconsistent values (the UI only ever showed a soft, unenforced warning) and the actual money paid out would silently disagree with what's displayed in the admin/sale panel. Now a hard validation error on both create and update.
2. `ensureCityWardColumns()`'s information-schema-check-then-ALTER pattern (added for the MySQL-8 fix above) isn't atomic — a genuinely concurrent double-run of `migrate.ts` (which does happen with some deploy setups) could see both processes attempt the same `ALTER` and crash the loser with a real duplicate-column/duplicate-key MySQL error. Fixed by catching exactly those two error codes and treating them as success, since that specific failure can only mean the desired end state was already reached by the other run.

A third, low-severity finding was reviewed and deliberately left as-is rather than fixed: after the UI split above, `MainSearchBar` and `FilterBar` are two separate client components that each independently read/write the URL's query string. In the rare case a user submits the address search and clicks a status pill in the same instant, the second update can be built from stale params and drop the first — UI-only, no data/money impact, and self-corrects on the next interaction. Reworking this would mean re-coupling the two bars into shared state, undoing the split that was just requested.

All rounds verified with `tsc --noEmit`, a production build, `eslint` (0 errors throughout, only pre-existing unrelated warnings), and live testing against a real MariaDB instance for every fix described above. See the standalone round-2 and round-3 QA reports for full before/after verification detail on each finding.
